...
# Local price cache populated by download_data.py. Avoids re-hitting
# yfinance (which rate-limits back-to-back requests, e.g. when looping
# over many pairs in multi_pair_backtest.py / window_optimize.py) for
# tickers we've already pulled.
REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
CACHE_DIR = os.path.join(REPO_ROOT, 'data', 'price_data')
...
def read_cached_close(ticker):
    """Return cached Close series for `ticker`, or None if not cached."""
...
class data_loader():
    ...
    def __init__(self):
        ...
        if 'start' not in PARAMS:
            raise ValueError(
                "PARAMS['start'] is not set. The training-window start date "
                "must be pinned explicitly in params.py so every script "
                "(main.py, experiment1.py, ...) loads identical history — "
                "beta and the OU mean depend on it, so an implicit default "
                "here would let the two silently drift apart.")
    ...
    def load_data(self, needs_fx=False):
        ...
        # ── Load (cache first, yfinance only if missing) ───────
        tickers_needed = ([self.T1, self.T2, 'USDINR=X']
                           if needs_fx
                           else [self.T1, self.T2])

        cached = {t: read_cached_close(t) for t in tickers_needed}
        if all(s is not None for s in cached.values()):
            raw = pd.concat(cached.values(), axis=1)
            raw = raw[raw.index >= pd.Timestamp(self.START)]
            print(f"  Loaded from local cache: {', '.join(tickers_needed)}")
        else:
            missing = [t for t, s in cached.items() if s is None]
            print(f"  Not cached locally, downloading via yfinance: {', '.join(missing)}")
            raw = yf.download(
                tickers_needed,
                start=self.START,
                auto_adjust=True,
