...
class plotting():
    ...
    def plot_all(self, df, feat, sig, pnl, equity, trades, stat_d, metrics,
                ...
                if edt in sig.index:
                    try:
                        ...
                        ax2.scatter(edt, np.clip(yv, -5, 5),
                                    color=c, s=25, zorder=5, alpha=0.9,
                                    marker='o')
                    except Exception:
                        pass
        ...
        out_path = os.path.join(FIGURES_DIR, 'NG_CL_statarb_v3.png')
    ...
    def print_trade_audit(self, trades, df):
        ...
        Columns expected in trades DataFrame (all written by backtest()):
            ...
            entry_z, exit_z, units, beta, leverage,
        ...
        total_notional = float(first['notional'])
        ...
        for idx, (_, t) in enumerate(trades.iterrows()):
            ...
            # Per-trade notional (read from trade log, not a global constant)
            tn = float(t['notional'])
        ...
        if mismatches:
            ...
            print(f"     exit_cost_dol accumulation.")
        else:
    ...
    def print_report(self, metrics, stat_d, trades, p, ou_mean,
        ...
        pair_tag  = f"{p.get('T1','?')} / {p.get('T2','?')}"
        lev_val   = trades['leverage'].iloc[0] if (trades is not None and len(trades) > 0
                                                     and 'leverage' in trades.columns) else p.get('leverage', '—')

        print(f"""
╔{'═'*66}╗
║  {pair_tag}  STAT-ARB  v3{tag:<38}║
╠{'═'*66}╣
║  STRUCTURAL
║    Hedge ratio β        : {beta_val:.4f}  (log(T1) − β×log(T2))
║    Notional cap         : 200,000  (hard ceiling per trade)
║    Leverage             : {lev_val if isinstance(lev_val,str) else f'{lev_val:.0f}'}x
╠{'═'*66}╣
        ...
        if trades is not None and len(trades) > 0:
            ...
            for _, t in trades.tail(20).iterrows():
                ...
                print(f"  {str(ed):>12}  {str(xd):>12}  {t['direction']:>6}  "
                      ...
                      f"{units:>5}  {pnl_dol:>9,.2f}  {t['exit_reason']}")
